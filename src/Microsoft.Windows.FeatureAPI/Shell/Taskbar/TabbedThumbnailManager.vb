...
Namespace Taskbar
	...
	Public Class TabbedThumbnailManager
		...
		''' </summary>
		Private _tabbedThumbnailCache As Dictionary(Of IntPtr, TabbedThumbnail)
		Private _tabbedThumbnailCacheWPF As Dictionary(Of UIElement, TabbedThumbnail)
		' list for WPF controls
		''' <summary>
		...
		''' </summary>
		Friend Sub New()
			_tabbedThumbnailCache = New Dictionary(Of IntPtr, TabbedThumbnail)()
			_tabbedThumbnailCacheWPF = New Dictionary(Of UIElement, TabbedThumbnail)()
		End Sub
		...
		Public Sub AddThumbnailPreview(preview As TabbedThumbnail)
		...
		Public Function GetThumbnailPreview(control As Control) As TabbedThumbnail
		...
		Public Function GetThumbnailPreview(windowsControl As UIElement) As TabbedThumbnail
		...
		Public Sub RemoveThumbnailPreview(preview As TabbedThumbnail)
			If preview Is Nothing Then
				Throw New ArgumentNullException("preview")
			End If
			...
			ElseIf _tabbedThumbnailCacheWPF.ContainsKey(preview.WindowsControl) Then
		...
		Public Sub RemoveThumbnailPreview(windowHandle As IntPtr)
			...
			Dim taskbarWindow As TaskbarWindow = TaskbarWindowManager.GetTaskbarWindow(windowHandle, TaskbarProxyWindowType.TabbedThumbnail)

			If taskbarWindow IsNot Nothing Then
				If TaskbarWindowManager._taskbarWindowList.Contains(taskbarWindow) Then
		...
		''' is to be removed from the taskbar</param>
		Public Sub RemoveThumbnailPreview(control As Control)
			If control Is Nothing Then
				Throw New ArgumentNullException("control")
			End If
		...
		''' is to be removed from the taskbar</param>
		Public Sub RemoveThumbnailPreview(windowsControl As UIElement)
			If windowsControl Is Nothing Then
				Throw New ArgumentNullException("windowsControl")
			End If
			...
			Dim taskbarWindow As TaskbarWindow = TaskbarWindowManager.GetTaskbarWindow(windowsControl, TaskbarProxyWindowType.TabbedThumbnail)
		...
		Public Sub SetActiveTab(preview As TabbedThumbnail)
			If preview Is Nothing Then
				Throw New ArgumentNullException("preview")
			End If
		...
		''' <exception cref="System.ArgumentException">If the control/window is not yet added to the tabbed thumbnails list</exception>
		Public Sub SetActiveTab(control As Control)
			If control Is Nothing Then
				Throw New ArgumentNullException("control")
			End If
		...
		''' <exception cref="System.ArgumentException">If the control/window is not yet added to the tabbed thumbnails list</exception>
		Public Sub SetActiveTab(windowsControl As UIElement)
			If windowsControl Is Nothing Then
				Throw New ArgumentNullException("windowsControl")
			End If
		...
		Public Function IsThumbnailPreviewAdded(preview As TabbedThumbnail) As Boolean
			If preview Is Nothing Then
				Throw New ArgumentNullException("preview")
			End If

			If preview.WindowHandle <> IntPtr.Zero AndAlso _tabbedThumbnailCache.ContainsKey(preview.WindowHandle) Then
				Return True
			ElseIf preview.WindowsControl IsNot Nothing AndAlso _tabbedThumbnailCacheWPF.ContainsKey(preview.WindowsControl) Then
				Return True
			End If
		...
		''' <returns>true if the tab is already added on the taskbar; otherwise, false.</returns>
		Public Function IsThumbnailPreviewAdded(control As Control) As Boolean
			If control Is Nothing Then
				Throw New ArgumentNullException("control")
			End If
		...
		''' <param name="control">The preview to locate on the taskbar's tabbed thumbnail list</param>
		''' <returns>true if the tab is already added on the taskbar; otherwise, false.</returns>
		Public Function IsThumbnailPreviewAdded(control As UIElement) As Boolean
			If control Is Nothing Then
				Throw New ArgumentNullException("control")
			End If
		...
		Public Sub InvalidateThumbnails()
			' Invalidate all the previews currently in our cache.
			' This will ensure we get updated bitmaps next time

			For Each thumbnail As TabbedThumbnail In _tabbedThumbnailCache.Values
				TaskbarWindowManager.InvalidatePreview(thumbnail.TaskbarWindow)
					' TODO: Investigate this, and why it needs to be called.
				thumbnail.SetImage(IntPtr.Zero)
			Next

			For Each thumbnail As TabbedThumbnail In _tabbedThumbnailCacheWPF.Values
				TaskbarWindowManager.InvalidatePreview(thumbnail.TaskbarWindow)
				thumbnail.SetImage(IntPtr.Zero)
			Next
		End Sub
		...
		Public Shared Sub ClearThumbnailClip(windowHandle As IntPtr)
		...
		Public Sub SetThumbnailClip(windowHandle As IntPtr, clippingRectangle As System.Nullable(Of Rectangle))
			If clippingRectangle Is Nothing Then
			...
			Dim rectPtr As IntPtr = Marshal.AllocCoTaskMem(Marshal.SizeOf(rect))
			Try
		...
		''' </param>
		Public Shared Sub SetTabOrder(previewToChange As TabbedThumbnail, insertBeforePreview As TabbedThumbnail)
			If previewToChange Is Nothing Then
				Throw New ArgumentNullException("previewToChange")
			End If

			Dim handleToReorder As IntPtr = previewToChange.TaskbarWindow.WindowToTellTaskbarAbout

			If insertBeforePreview Is Nothing Then
				TaskbarList.Instance.SetTabOrder(handleToReorder, IntPtr.Zero)
			Else
				Dim handleBefore As IntPtr = insertBeforePreview.TaskbarWindow.WindowToTellTaskbarAbout
