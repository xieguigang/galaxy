        ...
        Private Sub buttonRefreshTaskbarList_Click(ByVal sender As Object, ByVal e As EventArgs) Handles buttonRefreshTaskbarList.Click
            ...
            For Each item As Object In listBox1.SelectedItems
                Select Case item.ToString()
                    Case "Notepad"
                        childWindowJumpList.AddUserTasks(
                            New JumpListLink(Path.Combine(systemFolder, "notepad.exe"), "Open Notepad") With
                            {
                                .IconReference = New IconReference(Path.Combine(systemFolder, "notepad.exe"), 0)
                            })
                    Case "Calculator"
                        childWindowJumpList.AddUserTasks(
                            New JumpListLink(Path.Combine(systemFolder, "calc.exe"), "Open Calculator") With
                            {
                                .IconReference = New IconReference(Path.Combine(systemFolder, "calc.exe"), 0)
                            })
                    Case "Paint"
                        childWindowJumpList.AddUserTasks(
                            New JumpListLink(Path.Combine(systemFolder, "mspaint.exe"), "Open Paint") With
                            {
                                .IconReference = New IconReference(Path.Combine(systemFolder, "mspaint.exe"), 0)
                            })
                    Case "WordPad"
                        childWindowJumpList.AddUserTasks(
                            New JumpListLink(Path.Combine(programFilesFolder, "Windows NT\Accessories\wordpad.exe"), "Open WordPad") With
                            {
                    ...
                    Case "Windows Explorer"
                        childWindowJumpList.AddUserTasks(
                            New JumpListLink(Path.Combine(windowsFolder, "explorer.exe"), "Open Windows Explorer") With
                            {
                                .IconReference = New IconReference(Path.Combine(windowsFolder, "explorer.exe"), 0)
                            })
                    Case "Internet Explorer"
                        childWindowJumpList.AddUserTasks(
                            New JumpListLink(Path.Combine(programFilesFolder, "Internet Explorer\iexplore.exe"), "Open Internet Explorer") With
                            {
                                .IconReference = New IconReference(Path.Combine(programFilesFolder, "Internet Explorer\iexplore.exe"), 0)
                            })
                    Case "Control Panel"
                        childWindowJumpList.AddUserTasks(
                            New JumpListLink((CType(KnownFolders.ControlPanel, ShellObject)).ParsingName, "Open Control Panel") With
                            {
                                .IconReference = New IconReference(Path.Combine(windowsFolder, "explorer.exe"), 0)
                            })
                    Case "Documents Library"
                        If ShellLibrary.IsPlatformSupported Then
                            childWindowJumpList.AddUserTasks(
                                New JumpListLink(KnownFolders.DocumentsLibrary.Path, "Open Documents Library") With
                                {
                                    .IconReference = New IconReference(Path.Combine(windowsFolder, "explorer.exe"), 0)
                                })
                        End If
