...
Public Class DialogPromise
    ...
    Friend Sub New(previousForm As Form, previousResult As DialogResult, data As Object)
    ...
    ' 启动链式调用的入口方法
    Public Shared Function OpenDialog(Of T As {Form, New})() As DialogPromise
        Return OpenDialog(Of T)(Nothing)
    End Function

    Public Shared Function OpenDialog(Of T As {Form, New})(data As Object) As DialogPromise
        Dim form As New T()
    ...
    ' 链式调用下一个对话框
    Public Function ThenDialog(Of T As {Form, New})() As DialogPromise
        Return ThenDialog(Of T)(Function(f) True, Nothing)
    End Function

    Public Function ThenDialog(Of T As {Form, New})(condition As Func(Of Form, Boolean)) As DialogPromise
        Return ThenDialog(Of T)(condition, Nothing)
    End Function

    Public Function ThenDialog(Of T As {Form, New})(data As Object) As DialogPromise
        Return ThenDialog(Of T)(Function(f) True, data)
    End Function

    Public Function ThenDialog(Of T As {Form, New})(
        condition As Func(Of Form, Boolean),
    ...
    ' 处理最终结果
    Public Sub [Finally](action As Action(Of DialogResult, Form))
    ...
    Public Sub OnSuccess(action As Action(Of Form))
        If _previousResult = DialogResult.OK Then
    ...
    ' 辅助方法：设置表单数据
    Private Sub SetFormData(form As Form, data As Object)
        ' 这里可以根据需要实现数据传递逻辑
        ' 例如通过接口、反射或特定属性设置
